...
/**
 * This class defines simple methods to bind service names to URI addresses: list, lookup, unbind,
 * bind, rebind.
 *
 * @author Rui Claro
 */
public class ZKNaming {
  ...
  public void bind(ZKRecord record) throws ZKNamingException {
    ...
    try {
      connect();
      if (record == null) throw new IllegalArgumentException("ZKRecord cannot be null!");
      ...
      if (stat == null)
        zoo.create(
            record.getPath(),
            record.getURI().getBytes(),
            ZooDefs.Ids.OPEN_ACL_UNSAFE,
            CreateMode.PERSISTENT);
      else throw new ZKNamingException("zNode already exists. Use rebind.");
  ...
  public void unbind(ZKRecord record) throws ZKNamingException {
    ...
    try {
      ...
      stat = zoo.exists(record.getPath(), true);
      if (stat == null) throw new ZKNamingException("zNode not found.");
