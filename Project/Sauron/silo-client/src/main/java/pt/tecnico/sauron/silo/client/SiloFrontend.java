...
public class SiloFrontend {
   ...
   ***/
  public SiloFrontend(String zooHost, String zooPort, Integer timeout, Integer retries)
   ...
   ***/
  public SiloFrontend(
      String zooHost, String zooPort, String instance, Integer timeout, Integer retries)
      throws SiloFrontendException {
  ...
  public CamJoinResponse cam_join(CamJoinRequest request) {
    for (this.crrRetry = 0; this.crrRetry < this.maxRetry; this.crrRetry++) {
      ...
      try {
        CamJoinRequest req = CamJoinRequest.newBuilder(request).addAllPrevTS(this.prevTS).build();

        CamJoinResponse response =
            this.stub.withDeadlineAfter(this.timeout, TimeUnit.SECONDS).camJoin(req);
  ...
  public CamInfoResponse cam_info(CamInfoRequest request) {
    for (this.crrRetry = 0; this.crrRetry < this.maxRetry; this.crrRetry++) {
      ...
      try {
        CamInfoRequest req = CamInfoRequest.newBuilder(request).addAllPrevTS(this.prevTS).build();
        CamInfoResponse response =
            this.stub.withDeadlineAfter(this.timeout, TimeUnit.SECONDS).camInfo(req);
  ...
  public ReportResponse report(ReportRequest request) {
    for (this.crrRetry = 0; this.crrRetry < this.maxRetry; this.crrRetry++) {
      ...
      try {
        ReportRequest req = ReportRequest.newBuilder(request).addAllPrevTS(this.prevTS).build();
        ReportResponse response =
            this.stub.withDeadlineAfter(this.timeout, TimeUnit.SECONDS).report(req);
  ...
  public TrackResponse track(TrackRequest request) {
    for (this.crrRetry = 0; this.crrRetry < this.maxRetry; this.crrRetry++) {
      debug(String.format("Try %d of Report Request", (this.crrRetry + 1)));
      try {
        TrackRequest req = TrackRequest.newBuilder(request).addAllPrevTS(this.prevTS).build();
        TrackResponse response =
            this.stub.withDeadlineAfter(this.timeout, TimeUnit.SECONDS).track(req);
  ...
  public TrackMatchResponse trackMatch(TrackMatchRequest request) {
    for (this.crrRetry = 0; this.crrRetry < this.maxRetry; this.crrRetry++) {
      ...
      try {
        TrackMatchRequest req =
            TrackMatchRequest.newBuilder(request).addAllPrevTS(this.prevTS).build();
        TrackMatchResponse response =
            this.stub.withDeadlineAfter(this.timeout, TimeUnit.SECONDS).trackMatch(req);
  ...
  public TraceResponse trace(TraceRequest request) {
    for (this.crrRetry = 0; this.crrRetry < this.maxRetry; this.crrRetry++) {
      ...
      try {
        TraceRequest req = TraceRequest.newBuilder(request).addAllPrevTS(this.prevTS).build();
        TraceResponse response =
            this.stub.withDeadlineAfter(this.timeout, TimeUnit.SECONDS).trace(req);
        debug("TrackMatch Response was received");
        this.prevTS = response.getNewTSList();
        return response;
      } catch (StatusRuntimeException e) {
  ...
  public CtrlPingResponse ctrl_ping(CtrlPingRequest request) {
    for (this.crrRetry = 0; this.crrRetry < this.maxRetry; this.crrRetry++) {
      ...
      try {
        return this.stub.withDeadlineAfter(this.timeout, TimeUnit.SECONDS).ctrlPing(request);
      } catch (StatusRuntimeException e) {
        requestHandler(e);
      }
    }
  ...
  public CtrlClearResponse ctrl_clear(CtrlClearRequest request) {
    for (this.crrRetry = 0; this.crrRetry < this.maxRetry; this.crrRetry++) {
      ...
      try {
        return this.stub.withDeadlineAfter(this.timeout, TimeUnit.SECONDS).ctrlClear(request);
      } catch (StatusRuntimeException e) {
        requestHandler(e);
      }
    }
  ...
  public CtrlInitResponse ctrl_init(CtrlInitRequest request) {
    for (this.crrRetry = 0; this.crrRetry < this.maxRetry; this.crrRetry++) {
      ...
      try {
        return this.stub.withDeadlineAfter(this.timeout, TimeUnit.SECONDS).ctrlInit(request);
      } catch (StatusRuntimeException e) {
        requestHandler(e);
      }
    }
