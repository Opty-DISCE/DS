...
public class TrackTraceIT extends BaseIT {
  ...
  public static void oneTimeSetUp() {
    ...
    Camera camera = Camera.newBuilder().setName("Camera1337").setLocation(location).build();
    /* Create Observations */
    for (int i = 0; i < 2; i++) {
      persons.add(ObservedObject.newBuilder().setType(Type.PERSON).setId("" + i).build());
      cars.add(ObservedObject.newBuilder().setType(Type.CAR).setId("AA000" + i).build());
      timestamps.add(Timestamp.newBuilder().setSeconds(500 - i).build());
    }
    for (Timestamp ts : timestamps) {
      for (ObservedObject person : persons) {
        observations.add(
            Observation.newBuilder().setCamera(camera).setObject(person).setTs(ts).build());
      }
      for (ObservedObject car : cars) {
        observations.add(
            Observation.newBuilder().setCamera(camera).setObject(car).setTs(ts).build());
      }
    }
    /* SetUp Server */
    frontend.ctrl_init(
        CtrlInitRequest.newBuilder().addCameras(camera).addAllObservations(observations).build());
  }
  ...
  public void checkTrack_PERSON() {
    Observation obs =
        frontend
            .track(TrackRequest.newBuilder().setObject(persons.get(0)).build())
            .getObservation();
  ...
  public void checkTrack_CAR() {
    Observation obs =
        frontend.track(TrackRequest.newBuilder().setObject(cars.get(0)).build()).getObservation();
  ...
  public void checkTrackEmpty_NOTFOUND() {
    ObservedObject obj = ObservedObject.newBuilder().setType(Type.PERSON).setId("3").build();
    assertEquals(
        NOT_FOUND,
        assertThrows(
                StatusRuntimeException.class,
                () -> {
                  frontend.track(TrackRequest.newBuilder().setObject(obj).build());
                })
            .getStatus()
            .getCode());
  }
  ...
  public void checkTrackEmpty_INVALID() {
    ObservedObject obj = ObservedObject.newBuilder().setType(Type.CAR).setId("3").build();
    assertEquals(
        INVALID_ARGUMENT,
        assertThrows(
                StatusRuntimeException.class,
  ...
  public void checkTrackOneMatch_PERSON() {
    List<Observation> obsList =
        frontend
            .trackMatch(TrackMatchRequest.newBuilder().setObject(persons.get(0)).build())
            .getObservationsList();
    assertTrue(obsList.size() == 1 && obsList.contains(observations.get(0)));
  }
  ...
  public void checkTrackOneMatch_CAR() {
    List<Observation> obsList =
        frontend
            .trackMatch(TrackMatchRequest.newBuilder().setObject(cars.get(0)).build())
            .getObservationsList();
    assertTrue(obsList.size() == 1 && obsList.contains(observations.get(2)));
  }

  @Test
  /* Check Pattern '[0-9]*0' */
  public void checkTrackMatchPattern_L() {
    ObservedObject obj = ObservedObject.newBuilder().setType(Type.PERSON).setId("*0").build();
    List<Observation> obsList =
        frontend
            .trackMatch(TrackMatchRequest.newBuilder().setObject(obj).build())
            .getObservationsList();
    assertTrue(obsList.size() == 1 && obsList.contains(observations.get(0)));
  }
  ...
  public void checkTrackMatchPattern_R() {
    ObservedObject obj = ObservedObject.newBuilder().setType(Type.PERSON).setId("*").build();
    List<Observation> obsList =
        frontend
            .trackMatch(TrackMatchRequest.newBuilder().setObject(obj).build())
            .getObservationsList();
  ...
  public void checkTrackMatchPattern_M() {
    ObservedObject obj = ObservedObject.newBuilder().setType(Type.CAR).setId("AA*0").build();
    List<Observation> obsList =
        frontend
            .trackMatch(TrackMatchRequest.newBuilder().setObject(obj).build())
            .getObservationsList();
  ...
  public void checkTrackMatchPattern_LR() {
    ObservedObject obj = ObservedObject.newBuilder().setType(Type.PERSON).setId("*1*").build();
    List<Observation> obsList =
        frontend
            .trackMatch(TrackMatchRequest.newBuilder().setObject(obj).build())
            .getObservationsList();
    assertTrue(obsList.size() == 1 && obsList.contains(observations.get(1)));
  }

  @Test
  /* Check Pattern '[A-Z0-9]*0[A-Z0-9]*0' */
  public void checkTrackMatchPattern_LM() {
    ObservedObject obj = ObservedObject.newBuilder().setType(Type.CAR).setId("*0*0").build();
    List<Observation> obsList =
        frontend
            .trackMatch(TrackMatchRequest.newBuilder().setObject(obj).build())
            .getObservationsList();
  ...
  public void checkTrackMatchPattern_MR() {
    ObservedObject obj = ObservedObject.newBuilder().setType(Type.CAR).setId("A*0*").build();
    List<Observation> obsList =
        frontend
            .trackMatch(TrackMatchRequest.newBuilder().setObject(obj).build())
            .getObservationsList();
    assertTrue(
        obsList.size() == 2
            ...
            && obsList.contains(observations.get(3)));
  }

  @Test
  /* Check NOT_FOUND Exception | Try to TrackMatch a Non Observed Object */
  public void checkTrackMatchEmpty_NOTFOUND() {
    ObservedObject obj = ObservedObject.newBuilder().setType(Type.CAR).setId("B*").build();
    assertEquals(
        NOT_FOUND,
        assertThrows(
                StatusRuntimeException.class,
                () -> {
                  frontend.trackMatch(TrackMatchRequest.newBuilder().setObject(obj).build());
                })
            .getStatus()
            .getCode());
  }
  ...
  public void checkTrace_PERSON() {
    List<Observation> obsList =
        frontend
            .trace(TraceRequest.newBuilder().setObject(persons.get(0)).build())
            .getObservationsList();
    assertTrue(
        obsList.size() == 2
            && obsList.get(0).equals(observations.get(0))
            && obsList.get(1).equals(observations.get(4)));
  }
  ...
  public void checkTrace_CAR() {
    List<Observation> obsList =
        frontend
            .trace(TraceRequest.newBuilder().setObject(cars.get(0)).build())
            .getObservationsList();
    assertTrue(
        obsList.size() == 2
            && obsList.get(0).equals(observations.get(2))
            && obsList.get(1).equals(observations.get(6)));
  }
  ...
  public void checkTraceEmpty_NOTFOUND() {
    ...
    assertEquals(
        ...
        assertThrows(
                StatusRuntimeException.class,
                () -> {
                  frontend.trace(TraceRequest.newBuilder().setObject(obj).build());
                })
            .getStatus()
            .getCode());
  }
  ...
  public void checkTraceEmpty_INVALID() {
    ObservedObject obj = ObservedObject.newBuilder().setType(Type.PERSON).setId("AA0000").build();
    assertEquals(
        INVALID_ARGUMENT,
        assertThrows(
                StatusRuntimeException.class,
