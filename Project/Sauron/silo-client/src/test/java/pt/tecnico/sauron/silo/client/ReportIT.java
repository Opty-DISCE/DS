...
public class ReportIT extends BaseIT {
  ...
  public static void oneTimeSetUp() {
    ...
    Location location = Location.newBuilder().setLongitude(20).setLatitude(20).build();

    for (int i = 0; i < 3; i++) {
      cameras.add(Camera.newBuilder().setName("Camera" + i).setLocation(location).build());
      persons.add(ObservedObject.newBuilder().setType(PERSON).setId("12345" + i).build());
      cars.add(ObservedObject.newBuilder().setId("ABCD0" + i).setType(CAR).build());
    }
  }
  ...
  public void checkReport_PERSON() {
    ...
    Timestamp ts = Timestamp.newBuilder().setSeconds(instant.getEpochSecond()).build();

    assertTrue(
        observation.getCamera().equals(cameras.get(0))
  ...
  public void checkReportWrongCamName() {
    assertEquals(
        ...
        assertThrows(
                ...
                () -> {
                  frontend.report(
                      ReportRequest.newBuilder().setCamName("Camera3").addAllObjects(cars).build());
                })
            .getStatus()
            .getCode());
  }
  ...
  public void checkReportWrongId_PERSON() {
    ObservedObject person = ObservedObject.newBuilder().setId("12A3").setType(PERSON).build();
  ...
  public void checkReportWrongId_CAR() {
    ObservedObject car = ObservedObject.newBuilder().setId("123ABC").setType(CAR).build();
  ...
  public void checkReportWrongType() {
    ObservedObject object = ObservedObject.newBuilder().setId("ABCD12").setType(UNKNOWN).build();
