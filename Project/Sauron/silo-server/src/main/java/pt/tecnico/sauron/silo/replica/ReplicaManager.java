...
public class ReplicaManager {
  ...
  public ReplicaManager(
      Sauron sauron, String zooHost, String zooPort, String rootPath, Integer ID, Long sync) {
    this.ID = ID;
  ...
  private int execUpdate(Update update) throws SauronException {
    ...
    boolean added;
    synchronized (this.updateLog) {
      added = this.updateLog.add(update);
    }
    if (!added) {
        /* Update is duplicate */
      return -1;
    ...
    synchronized (this.valueTS) {
      this.valueTS.increment(update.getID());
    }
  ...
  private class UpdateSender extends TimerTask {
    ...
    public void run() {
      ...
      ZKRecord[] records;
      try {
        records = this.zkNaming.listRecords(this.rootPath).toArray(ZKRecord[]::new);
