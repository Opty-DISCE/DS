...
public class SauronServiceImpl extends SauronImplBase {
  ...
  @Override
  public void camJoin(
      Silo.CamJoinRequest request, StreamObserver<Silo.CamJoinResponse> responseObserver) {
    try {
      ...
      responseObserver.onNext(
          Silo.CamJoinResponse.newBuilder().addAllNewTS(toSiloVecTS(newTS)).build());
  ...
  @Override
  public void camInfo(
      Silo.CamInfoRequest request, StreamObserver<Silo.CamInfoResponse> responseObserver) {
    try {
      ...
      responseObserver.onNext(
          Silo.CamInfoResponse.newBuilder()
  ...
  @Override
  public void report(
      Silo.ReportRequest request, StreamObserver<Silo.ReportResponse> responseObserver) {
    try {
      ...
      ReportUpdate report =
          new ReportUpdate(
              this.ID, prevTS, request.getCamName(), objects, instant.getEpochSecond());
      VecTimestamp newTS = this.repManager.update(report);

      debug("Responding to a 'report' Request");
      responseObserver.onNext(
          Silo.ReportResponse.newBuilder().addAllNewTS(toSiloVecTS(newTS)).build());
      responseObserver.onCompleted();
    } catch (SauronException e) {
  ...
  @Override
  public void track(
      Silo.TrackRequest request, StreamObserver<Silo.TrackResponse> responseObserver) {
    try {
  ...
  public void trackMatch(
      Silo.TrackMatchRequest request, StreamObserver<Silo.TrackMatchResponse> responseObserver) {
    try {
      ...
      List<Observation> observations =
          this.sauron.trackMatch(object.getId(), toDomainType(object.getType()));

      debug("Responding to a 'trackMatch' Request");
      responseObserver.onNext(
          Silo.TrackMatchResponse.newBuilder()
  ...
  @Override
  public void trace(
      Silo.TraceRequest request, StreamObserver<Silo.TraceResponse> responseObserver) {
    try {
  ...
  public void ctrlPing(
      Silo.CtrlPingRequest request, StreamObserver<Silo.CtrlPingResponse> responseObserver) {
    try {
      ...
      responseObserver.onNext(
          Silo.CtrlPingResponse.newBuilder()
  ...
  public void ctrlInit(
      Silo.CtrlInitRequest request, StreamObserver<Silo.CtrlInitResponse> responseObserver) {
    try {
      ...
      responseObserver.onNext(Silo.CtrlInitResponse.newBuilder().build());
      responseObserver.onCompleted();
    } catch (SauronException e) {
  ...
  public void ctrlClear(
      Silo.CtrlClearRequest request, StreamObserver<Silo.CtrlClearResponse> responseObserver) {
    ...
    responseObserver.onNext(Silo.CtrlClearResponse.newBuilder().build());
    responseObserver.onCompleted();
  }
  ...
  public void updateReplica(
      Silo.UpdateRequest request, StreamObserver<Silo.UpdateResponse> responseObserver) {
    ...
    responseObserver.onNext(
        Silo.UpdateResponse.newBuilder().addAllUpdates(toSiloUpdates(updateLog)).build());
    responseObserver.onCompleted();
  }
}
