...
public class Sauron {
   ...
   ***/
  public void cam_report(String cam_name, List<ObservedObject> objects, Long seconds)
        ...
        .orElseThrow(
            () ->
                new SauronException(
                    NOT_FOUND,
                    ID_NOT_FOUND,
                    getObjectId(observedObject),
                    observedObject.getType().toString()));
  }
   ...
   ***/
  private synchronized List<Observation> patternMatches(
      Set<ObservedObject> objects, String id, Type type, String pattern) throws SauronException {

    String regex = id.replace("*", pattern);

    Set<ObservedObject> objectMatches =
        objects.stream()
   ...
   ***/
  public synchronized List<Observation> trace(ObservedObject observedObject)
      throws SauronException {

    Set<Observation> observations = getObjectObservations(observedObject);

    List<Observation> observationTrace =
        observations.stream()
            .filter(
                entry -> getObjectId(observedObject).equals(getObjectId(entry.getObservedObject())))
            .sorted()
  ...
  private Set<Observation> getObjectObservations(ObservedObject object) throws SauronException {
    ...
    throw new SauronException(
        NOT_FOUND, ID_NOT_FOUND, getObjectId(object), object.getType().toString());
  }
   ...
   ***/
  public void ctrl_init(List<Camera> camerasList, List<Observation> observationsList)
      throws SauronException {
    ...
    for (Observation observation : observationsList) {
      synchronized (this.cameras) {
        if (!cameras.containsKey(observation.getCamera().getName())) {
          throw new SauronException(
              NOT_FOUND, CAMERA_NAME_NOT_FOUND, observation.getCamera().getName());
        }
      }
  ...
  public List<Observation> getObservations() {
    ...
    synchronized (this.cars) {
      observations =
          this.cars.values().stream().flatMap(Collection::stream).collect(Collectors.toList());
    }
    synchronized (this.persons) {
      observations.addAll(
          this.persons.values().stream().flatMap(Collection::stream).collect(Collectors.toList()));
    }
    return observations;
  }
