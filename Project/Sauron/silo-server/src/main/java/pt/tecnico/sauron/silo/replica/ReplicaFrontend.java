...
public class ReplicaFrontend {
  ...
  private static final Integer timeout =
      Integer.parseInt(SiloServerApp.getProps().getProperty("timeout"));
  private static final Integer retries =
      Integer.parseInt(SiloServerApp.getProps().getProperty("retries"));
  private int retry = 0;
  ...
  public void updateReplica(VecTimestamp valueTS) {
    ...
    Silo.UpdateRequest request =
        Silo.UpdateRequest.newBuilder().addAllValueTS(toSiloVecTS(valueTS)).build();

    ListenableFuture<Silo.UpdateResponse> listener =
        stub.withDeadlineAfter(timeout, TimeUnit.SECONDS).updateReplica(request);
