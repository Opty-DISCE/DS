...
public class EyeApp {
  ...
  private static void parseLine(String line) {
    ...
    error(String.format("Command: '%s' not found!%n" + "For +Details type 'help'!", line));
  }
  ...
  private static void register(double latitude, double longitude) {
    ...
    Camera camera = Camera.newBuilder().setName(name).setLocation(location).build();
    try {
      /* Camera Registration */
      debug(
          String.format(
              "Registering Camera '%s' at Location (%f, %f)",
              name, location.getLatitude(), location.getLongitude()));
      frontend.cam_join(CamJoinRequest.newBuilder().setCamera(camera).build());
  ...
  private static void infoDisplay() {
    ...
    try {
      Location location =
          frontend.cam_info(CamInfoRequest.newBuilder().setName(name).build()).getLocation();
      System.out.printf(
          "info: Camera '%s' with Location (%f, %f)%n",
          name, location.getLatitude(), location.getLongitude());
    } catch (StatusRuntimeException e) {
      if (e.getStatus().getCode() == UNAVAILABLE) {
        error("Server Offline!");
  ...
  private static void stackDisplay() {
    ...
    for (ObservedObject object : stack) {
      System.out.printf("stack: %s '%s'%n", object.getType(), object.getId());
    }
  }
  ...
  private static void parseObservedObject(String type, String id) {
    ...
    switch (type.toUpperCase()) {
      case "PERSON":
        ...
        /* ObservedObject Push */
        if (stack.add(ObservedObject.newBuilder().setType(Type.PERSON).setId(id).build()))
          debug("Pushed!");
        break;
      case "CAR":
        /* Check Car ID */
        ...
        /* ObservedObject Push */
        if (stack.add(ObservedObject.newBuilder().setType(Type.CAR).setId(id).build()))
          debug("Pushed!");
        break;
      default:
        error(
            String.format("Invalid Object with Type: '%s'!%n" + "For +Details type 'help'!", type));
    }
  }
  ...
  private static void report() {
    ...
    try {
      frontend.report(ReportRequest.newBuilder().setCamName(name).addAllObjects(stack).build());
