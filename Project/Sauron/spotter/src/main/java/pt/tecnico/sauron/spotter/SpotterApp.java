...
public class SpotterApp {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      if (args.length == 2) {
        ...
        error(
            String.format(
                "%s Arguments: Zoo Host, Zoo Port, <Optional> Instance",
                SpotterApp.class.getSimpleName()));
        return;
      }
    } catch (IOException e) {
    ...
    while (scanner.hasNextLine()) {
      try {
        ...
        if (e.getStatus().getCode() == NOT_FOUND || e.getStatus().getCode() == INVALID_ARGUMENT) {
          System.out.println();
  ...
  private static void spotCar(String id) {
    if (id.length() > 6) {
      ...
      track(ObservedObject.newBuilder().setId(id).setType(Type.CAR).build());

    } else if (id.contains("*")) {
      debug(String.format("Scanned car with partial id: '%s'!", id));
      trackMatch(ObservedObject.newBuilder().setId(id).setType(Type.CAR).build());
  ...
  private static void spotPerson(String id) {
    try {
      if (id.matches("\\d+")) {
        ...
        track(ObservedObject.newBuilder().setId(id).setType(Type.PERSON).build());

      } else if (id.contains("*")) {
        debug(String.format("Scanned Person with partial id: '%s'!", id));
        trackMatch(ObservedObject.newBuilder().setId(id).setType(Type.PERSON).build());
  ...
  private static void track(ObservedObject object) {
    TrackResponse response = silo.track(TrackRequest.newBuilder().setObject(object).build());
    if (response.getObservation() != null) printObservation(response.getObservation());
  }
  ...
  private static void trackMatch(ObservedObject object) {
    TrackMatchResponse response =
        silo.trackMatch(TrackMatchRequest.newBuilder().setObject(object).build());
    ...
    switch (object.getType()) {
      case CAR:
        observations =
            observations.stream()
                .sorted((o1, o2) -> compareByIdCar(o1.getObject().getId(), o2.getObject().getId()))
      ...
      case PERSON:
        observations =
            observations.stream()
                .sorted(
                    (o1, o2) -> compareByIdPerson(o1.getObject().getId(), o2.getObject().getId()))
                .collect(Collectors.toList());
        break;
    }
  ...
  private static void trace(ObservedObject object) {
    TraceResponse response = silo.trace(TraceRequest.newBuilder().setObject(object).build());
  ...
  private static boolean checkCarId(String carId) {
    return carId.matches(
        "\\d{2}[A-Z]{4}|\\d{4}[A-Z]{2}|[A-Z]{2}\\d{4}"
  ...
  private static void command_control(String[] params) {
    ...
    error(
        String.format(
            "Invalid Command: '%s' with number of parameters: %d", params[0], params.length));
  }
  ...
  private static void ctrl_ping() {
    ...
    List<Camera> cameras =
        response.getCamerasList().stream()
            .sorted(Comparator.comparing(Camera::getName))
            .collect(Collectors.toList());
    for (Camera camera : cameras) {
      System.out.printf(
          ...
          camera.getLocation().getLongitude());
    }
    System.out.println();
    ...
    List<Observation> observations =
        response.getObservationsList().stream()
            .sorted((o1, o2) -> compareTs(o1.getTs(), o2.getTs()))
            .collect(Collectors.toList());
    ...
    try {
      switch (params[0]) {
        ...
        case "observation":
          ...
          Timestamp ts = Timestamp.newBuilder().setSeconds(time.getEpochSecond()).build();
          Camera camera = createCamera(params[4], params[5], params[6]);
          Observation observation =
              Observation.newBuilder().setCamera(camera).setTs(ts).setObject(object).build();
          request.addObservations(observation);
          return;
      }
      error(
          String.format(
              "Init - On file - Invalid Command : '%s' with number of parameters: %d",
              params[0], params.length));
    } catch (NumberFormatException e) {
      error("Init - latitude or longitude: Out of bounds!");
    }
  }
  ...
  private static Camera createCamera(String cameraName, String latitudeStr, String longitudeStr) {
    ...
    Location location = Location.newBuilder().setLatitude(latitude).setLongitude(longitude).build();
    return Camera.newBuilder().setName(cameraName).setLocation(location).build();
  }
  ...
  private static void ctrl_help() {
    ...
    System.out.println(
        ">>> spot <type> <id>      : Finds most recent observation for the matching objects");
    ...
    System.out.println(
        ">>> camera <cameraName> <latitude> <longitude>                              : Creates a camera");
    System.out.println(
        ">>> observation <type> <id> <timestamp> <cameraName> <latitude> <longitude> : Creates Observation");
    System.out.println();
  }
  ...
  private static ObservedObject createObservedObject(String type, String id) {
    ...
    switch (type.toUpperCase()) {
      case "PERSON":
        object = ObservedObject.newBuilder().setId(id).setType(Type.PERSON).build();
        break;
      case "CAR":
        object = ObservedObject.newBuilder().setId(id).setType(Type.CAR).build();
    }
    return object;
  }
